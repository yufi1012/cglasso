...
"""
orthogonal nonnegative matrix tri-factorizations with spatial continuity
:emp_env:S
:H prior:k-means + .2
:M prior:H.T*S*H
:H threshold:
...
##################################################################################################
class onmtf_spa(expModel):
    name = "onmtf_spa"
    desc = "orthogonal nonnegative matrix tri-factorizations with spatial continuity"
    category = "cluster"
    
    def __init__(self,slice_index,sig2,beta,n_cluster,clus_threshold,assign_labels,random_state=1991):
        super(onmtf_spa,self).__init__('onmtf_spa','orthogonal nonnegative matrix tri-factorizations')
    ...
    def predict(self,data):
        ...
        #get Theta matrix
        distance_mat = []
        for i in xrange(len(self.slice_index[0])):
            distance_i = []
            for j in xrange(len(self.slice_index[0])):
                dis = (self.slice_index[0][i]-self.slice_index[0][j])**2+(self.slice_index[1][i]-self.slice_index[1][j])**2
                dis = np.exp(dis/float(2*self.sig2))
                distance_i.append(dis)
            distance_mat.append(distance_i)
    
        theta = np.mat(distance_mat)
        ...
        while iteration < iterations:
            while iteration_H < 50:
                H_new = iter_H(H_prior,S,M_prior,theta,self.beta)
...
########################################################################################
def iter_H(H_prior,S,M_prior,theta,beta):#function of iteration H
    H = copy.deepcopy(H_prior)
    M = copy.deepcopy(M_prior)
    numerator = S.T*H*M
    denominator = H*H.T*S*H*M + beta*theta*H
